...
class PSO:
    def __init__(self, n_particles=50, max_iter=200, w=0.7, c1=1.5, c2=1.5):
    ...
    def evaluate(self, particle, problem, predictor_fn, penalty_lambda):
        ...
        weighted_fitness = (
            ...
            + weights["schedule"] * norm_schedule
        )
    ...
    def optimize(self, problem, predictor_fn):
        ...
        for iteration in range(self.max_iter):
            ...
            for i in range(self.n_particles):
                r1 = np.random.uniform(0, 1, size=velocity[i].shape)
                r2 = np.random.uniform(0, 1, size=velocity[i].shape)
                velocity[i] = (
                    self.w * velocity[i]
